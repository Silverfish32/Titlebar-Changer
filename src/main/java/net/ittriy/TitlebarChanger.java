...
public class TitlebarChanger implements ClientModInitializer {
	...
	public static void applyChanges(Window window) {
		...
		if (TitleBarSettings.theme == 1) {
			...
			memTheme.setInt(0, 0);
			memTitleBarColor.setInt(0, ColorConverter.rgbToHex(
					configFile.getConfig().getTitleBarColor().getR(),
					configFile.getConfig().getTitleBarColor().getG(),
					configFile.getConfig().getTitleBarColor().getB()
			));
			memTitleBarTextColor.setInt(0, ColorConverter.rgbToHex(
					configFile.getConfig().getTitleBarTextColor().getR(),
					configFile.getConfig().getTitleBarTextColor().getG(),
					configFile.getConfig().getTitleBarTextColor().getB()
			));
			memTitleBarStrokeColor.setInt(0, ColorConverter.rgbToHex(
					configFile.getConfig().getTitleBarStrokeColor().getR(),
					configFile.getConfig().getTitleBarStrokeColor().getG(),
